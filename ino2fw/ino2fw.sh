...
usage() {
  exec >&2
  [ -n "${1}" ] && printf "Error : ${1}\n"
  printf "Usage : $(basename "${0}") [options] {sketch_dir|sketch_file.ino}\n"
...
while getopts b:t:l:d:vh opt; do case "${opt}" in
  b) BUILD_PATH="${OPTARG}";;
  t) [ -e "${OPTARG}" ] || usage; FW_TILE="${OPTARG}";;
  l) FW_LABEL="${OPTARG}";;
  d) FW_DESC="${OPTARG}";;
...
[ -e "${SKETCH_FILE}" ] || usage "No '$(basename "${SKETCH_FILE}")' file found in '${SKETCH_DIR}'"
...
log "moving fw file to '${SKETCH_DIR}'"
mv firmware.fw "${SKETCHDIR}/$(basename "${SKETCH_DIR}").fw" || exit 32
